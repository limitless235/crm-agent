...
export const AuthComponent = ({
    ...
    const handleAuthSubmit = async (e?: React.FormEvent) => {
        ...
        if (mode === 'signup' && authStep !== 'confirmPassword') {
            handleProgressStep();
            return;
        }

        if (mode === 'login' && authStep !== 'password') {
            handleProgressStep();
            return;
        }
        ...
        <AnimatePresence>
            {modalStatus !== 'closed' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2 overflow-hidden shadow-2xl">
                        <div className="absolute inset-0 opacity-10 pointer-events-none"><GradientBackground /></div>
                        {(modalStatus === 'error' || modalStatus === 'success') && <button onClick={closeModal} className="absolute top-2 right-2 p-1 text-muted-foreground hover:text-foreground transition-colors z-20"><X className="w-5 h-5" /></button>}
                        {modalStatus === 'error' && <>
                            <AlertCircle className="w-12 h-12 text-destructive relative z-10" />
                            <p className="text-lg font-medium text-foreground relative z-10 text-center">{modalErrorMessage}</p>
                            <GlassButton onClick={closeModal} size="sm" className="mt-4 relative z-10">Try Again</GlassButton>
                        </>}
                        {modalStatus === 'loading' &&
                            <div className="relative z-10 h-[100px] flex flex-col items-center justify-center">
    ...
    return (
        <div className="bg-background min-h-screen w-screen flex flex-col selection:bg-primary/20">
            <style>{`
            ...
            <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden")}>
                ...
                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                    <AnimatePresence mode="wait">
                        ...
                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{mode === 'login' ? 'Enter access key' : 'Create your password'}</p></div></BlurFade>
                            <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{mode === 'login' ? 'Enter your credentials to continue.' : 'Your password must be at least 6 characters long.'}</p></BlurFade>
                        </motion.div>}
                    ...
                    <form onSubmit={handleAuthSubmit} className="w-[300px] space-y-6">
                        <AnimatePresence>
                                    ...
                                    {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                        <div className="relative w-full">
                                            ...
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                ...
                                                <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0")}><GlassButton type="button" onClick={mode === 'login' ? handleAuthSubmit : handleProgressStep} size="icon" aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground">{mode === 'login' ? <ArrowRight className="w-5 h-5" /> : <ArrowRight className="w-5 h-5" />}</GlassButton></div>
                                            </div></div>
                                        </div>
                ...
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 text-center">
                    <p className="text-xs text-muted-foreground">
                        ...
                        <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} className="font-bold text-foreground hover:underline">
